import prakBottom from '../assets/prak-bottom.png'
import prakTopBg from '../assets/prak-top-bg.png'

export const Praktisch = () => {
	document.title = 'Praktische info – KineKriebels'
	return (
		<div className="flex flex-col items-center">
			{/* Hero */}
			<div className="relative w-full page-hero">
				<div className="pt-16">
					<img
						src={prakTopBg}
						className="w-full h-auto object-cover"
						alt="Background"
						loading="eager"
					/>
				</div>
			</div>

			{/* Intro */}
			<div className="w-full section-warm">
				<div className="max-w-4xl mx-auto page-section">
					<div className="animate-fadeInUp">
						<div className="accent-line mb-6"></div>
						<h1 className="font-display font-bold text-4xl md:text-5xl text-gray-900 mb-4">
							Praktische info
						</h1>
						<p className="text-xl md:text-2xl text-gray-500 leading-relaxed">
							Alle belangrijke info op een rijtje.
						</p>
					</div>

					<div
						className="animate-fadeInUp mt-8"
						style={{ animationDelay: '0.1s' }}
					>
						<p className="text-lg md:text-xl leading-relaxed text-gray-600">
							Hieronder vind je alle praktische informatie over de werking van de
							praktijk. Heb je toch nog vragen? Aarzel niet om contact op te nemen –
							ik help je graag verder.
						</p>
					</div>
				</div>
			</div>

			{/* Info Cards */}
			<div className="w-full bg-white">
				<div className="max-w-4xl mx-auto page-section">
					{/* Appointments */}
					<div
						className="quote-block mb-10 animate-fadeInUp"
						style={{ animationDelay: '0.15s' }}
					>
						<div className="flex items-start gap-4 mb-4">
							<span className="text-3xl">🗓️</span>
							<h2 className="font-display font-bold text-2xl text-gray-900">
								Afspraken
							</h2>
						</div>
						<p className="text-lg leading-relaxed text-gray-600">
							Alle sessies gaan door{' '}
							<span className="font-bold text-gray-900">op afspraak</span>. Zo kan ik
							voldoende tijd en aandacht voorzien voor elk kind.
						</p>
					</div>

					{/* Location & Hours Grid */}
					<div className="grid md:grid-cols-2 gap-6 mb-10">
						<div
							className="info-card animate-fadeInUp"
							style={{ animationDelay: '0.2s' }}
						>
							<div className="flex items-start gap-3 mb-4">
								<span className="text-3xl">📍</span>
								<h3 className="font-display font-bold text-xl text-gray-900">
									Locatie
								</h3>
							</div>
							<div className="space-y-1.5 text-lg text-gray-600">
								<p className="font-semibold text-gray-900">KineKriebels</p>
								<p>Katteweidelaan 84</p>
								<p>3590 Diepenbeek</p>
							</div>
						</div>

						<div
							className="info-card animate-fadeInUp"
							style={{ animationDelay: '0.25s' }}
						>
							<div className="flex items-start gap-3 mb-4">
								<span className="text-3xl">⏰</span>
								<h3 className="font-display font-bold text-xl text-gray-900">
									Openingsuren
								</h3>
							</div>
							<div className="space-y-1.5 text-lg text-gray-600">
								<p>Enkel op afspraak</p>
							</div>
						</div>
					</div>

					{/* Session Duration */}
					<div
						className="info-card mb-10 animate-fadeInUp"
						style={{ animationDelay: '0.3s' }}
					>
						<div className="flex items-start gap-4 mb-4">
							<span className="text-3xl">⏱️</span>
							<h2 className="font-display font-bold text-2xl text-gray-900">
								Duur van de sessie
							</h2>
						</div>
						<ul className="space-y-3 text-lg text-gray-600">
							<li className="flex items-start gap-3">
								<span className="text-pink-400 mt-1.5">●</span>
								<span>
									Een sessie duurt{' '}
									<span className="font-bold text-gray-900">30 minuten</span>
								</span>
							</li>
							<li className="flex items-start gap-3">
								<span className="text-pink-400 mt-1.5">●</span>
								<span>
									Intakegesprek of eerste kennismaking kan meer tijd in beslag
									nemen
								</span>
							</li>
						</ul>
					</div>

					{/* Pricing */}
					<div
						className="info-card animate-fadeInUp"
						style={{ animationDelay: '0.35s' }}
					>
						<div className="flex items-start gap-4 mb-4">
							<span className="text-3xl">💶</span>
							<h2 className="font-display font-bold text-2xl text-gray-900">
								Tarieven
							</h2>
						</div>
						<p className="text-lg leading-relaxed text-gray-600">
							Als kinesitherapeut ben ik gedeconventioneerd. Terugbetaling gebeurt
							volgens de tarieven van de mutualiteit.
						</p>
					</div>
				</div>
			</div>

			{/* Bottom illustration */}
			<div className="py-8 section-warm w-full flex justify-center">
				<img
					src={prakBottom}
					className="h-[25vh] mx-auto"
					alt="Decoratie"
					loading="lazy"
				/>
			</div>
		</div>
	)
}
